...
- [x] First importer batches: `fuel-records`, `adblue-records`, `depot-stock`, `equipment-assets`, `tyre-assets`, `purchase-requests`, `defect-damage-link`, `dispatch-assignment-gates`, `driver-write-guards`, `incident-workflow` (34 company-scoped `admin` importers remain)
