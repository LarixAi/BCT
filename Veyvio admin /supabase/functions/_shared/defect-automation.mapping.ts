/**
 * Pure defect-automation rule matching (no DB).
 * Used by maybeCreateExceptionForDefect to decide whether to raise an operational exception.
 */
...
/** True when a defect must raise a durable Command operational exception. */
export function defectCreatesOperationalException(input: {
  severity?: string | null
  category?: string | null
  component?: string | null
}): boolean {
  return rulesTriggeredByDefect(input).some((rule) => rule.actions.includes('create_exception'))
}
