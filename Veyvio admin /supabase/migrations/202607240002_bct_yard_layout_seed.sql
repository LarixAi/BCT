-- BCT Main Depot — Live Yard Map seed (run after company + depot exist)
-- Matches shared/veyvio-yard/bct-main-depot.ts geometry (Phase 1)
...
begin
  ...
  where external_reference = 'BCT' or trading_name ilike '%Brent Community Transport%'
  order by created_at asc limit 1;

  if v_company_id is null then
    raise notice 'BCT company not found — skip yard layout seed';
    return;
  end if;
  ...
  if v_layout_id is not null then
    raise notice 'BCT yard layout already seeded';
    return;
  end if;
  ...
  values
    (v_company_id, v_depot_id, v_layout_id, 'Column 3 LIFO', 'front_first', 'north'),
    (v_company_id, v_depot_id, v_layout_id, 'Centre row LIFO', 'front_first', 'east')
  returning id into v_lifo_a;

  select id into v_lifo_b from public.lifo_groups where layout_id = v_layout_id and name = 'Centre row LIFO';

  -- Bays 1–26 with coordinates from shared layout (abbreviated insert via generate_series + case)
  ...
  raise notice 'BCT Live Yard Map seeded for depot %', v_depot_id;
end $$;
