-- Link admin@veyvio.test to Brent Community Transport (BCT) for Gate 1 smoke / pilot prep.
...
begin
  ...
  from auth.users
  where lower(email) = lower('admin@veyvio.test')
  limit 1;

  if v_user_id is null then
    raise notice 'User admin@veyvio.test not found in auth.users — skip BCT membership link';
    return;
  end if;

  insert into public.users (id, email, first_name, last_name)
  values (v_user_id, 'admin@veyvio.test', 'Alex', 'Command')
  ...
  raise notice 'Linked admin@veyvio.test (%) to BCT company % depot %',
