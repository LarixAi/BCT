-- Link veyvio@outlook.com to Brent Community Transport (BCT) with Yard access to BCT Main Depot.
...
begin
  ...
  from auth.users
  where lower(email) = lower('veyvio@outlook.com')
  limit 1;

  if v_user_id is null then
    raise notice 'User veyvio@outlook.com not found in auth.users — skip BCT membership link';
    return;
  end if;

  insert into public.users (id, email, first_name, last_name)
  values (v_user_id, 'veyvio@outlook.com', 'Kenny', 'Laing')
  ...
  raise notice 'Linked veyvio@outlook.com (%) to BCT company % depot %',
