-- Tenant table register for Multi-Tenant Readiness Audit / Wave 3F-B
with tables as (
  select c.oid, n.nspname as schema_name, c.relname as table_name,
         c.relrowsecurity as rls_enabled, c.relforcerowsecurity as rls_forced
  from pg_class c
  join pg_namespace n on n.oid = c.relnamespace
  where n.nspname in ('public','storage','private','cost_control') and c.relkind = 'r'
),
cols as (
  ...
  where table_schema in ('public','storage','private','cost_control')
...
pols as (
  ...
  from pg_policies
  where schemaname in ('public','storage','private','cost_control')
