...
export function DriverSafetyTab({ driverId }: { driverId: string }) {
  ...
  if (isLoading) return <p className="text-sm text-slate-500">Loading incident history…</p>
  ...
  return (
    ...
    <SectionCard title="Safety and incidents" description="Restricted summaries — allegations are not proven misconduct until investigation is complete">
      {incidents.length === 0 ? (
        <p className="text-sm text-slate-500">No incidents linked to this driver.</p>
      ) : (
        <ul className="divide-y divide-slate-100" data-testid="driver-incidents">
          {incidents.map((inc) => (
            <li key={inc.id} className="py-3 first:pt-0">
              <div className="flex flex-wrap items-start justify-between gap-2">
                <div>
                  <p className="font-mono text-xs text-slate-500">{inc.incidentRef}</p>
                  ...
                  {inc.outcomeSummary && <p className="mt-1 text-sm text-slate-700">{inc.outcomeSummary}</p>}
                </div>
