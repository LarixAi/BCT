    ...
    <div className="space-y-6">
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-8">
        {ATTENTION_CARDS.map((card) => {
          ...
          return (
            ...
            >
              <p className={`text-xl font-bold tabular-nums ${critical ? 'text-red-800' : 'text-slate-900'}`}>{value}</p>
                    ...
                    {items.map((item) => (
                      <li key={item.id} className="flex flex-wrap items-start justify-between gap-3 px-3 py-3 text-sm">
                        <div>
                          ...
                          {item.fleetNumber && <span className="text-slate-500"> · {item.fleetNumber}</span>}
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <SectionCard title="Upcoming 30 days" description="PMI, service, MOT and workshop bookings">
          {upcoming.length === 0 ? (
            <p className="text-sm text-slate-500">Nothing scheduled in the next 30 days.</p>
          ) : (
            <ul className="space-y-2 text-sm">
              {upcoming.map((e) => (
                <li key={e.id} className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-50 py-1.5">
                  <span className="flex items-center gap-2">
                    ...
                    <span className="text-slate-600">{e.title.replace(` — ${e.registrationNumber}`, '')}</span>
                  </span>
                  <span className="text-xs tabular-nums text-slate-500">{e.date.slice(0, 10)}</span>
                </li>
              ))}
            </ul>
          )}
        ...
        <div key={group.title}>
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">
            {group.cards.map((card) => {
              const value = hub.summary.fleetAvailability[card.id as keyof typeof hub.summary.fleetAvailability]
              return (
                <button
                  key={card.id}
                  type="button"
      ...
      <SectionCard title="Fleet register" description="Filter from the cards above">
        <input
          ...
          className="mb-3 w-full max-w-md rounded-lg border border-slate-200 px-3 py-1.5 text-sm"
        />
        <div className="overflow-x-auto">
          <table className="w-full min-w-[800px] text-left text-sm">
            ...
            <tbody>
              {fleetRows.map((r) => (
                <tr key={r.vehicleId} className="border-b border-slate-50">
                  <td className="py-2 pr-3">
                    <Link to={`/vehicles/${r.vehicleId}?tab=Maintenance`} className="font-medium text-command-600 hover:underline">
                      {r.registrationNumber}
                    </Link>
                  </td>
                  <td className="py-2 pr-3 text-slate-600">{r.depot}</td>
                  <td className="py-2 pr-3">
                    <StatusPill status={r.maintenanceStatus} />
                  </td>
                  <td className="py-2 pr-3 text-slate-600">{r.currentIssue ?? '—'}</td>
                  <td className="py-2 pr-3 text-xs text-slate-600">{r.nextServiceDate?.slice(0, 10) ?? '—'}</td>
        ...
        : type === 'retorque'
          ...
          : type === 'work_order'
            ...
            : type === 'inspection'
              ...
              : 'SVC'
  return (
    <span className="inline-flex min-w-[2.5rem] justify-center rounded bg-slate-100 px-1 py-0.5 text-[10px] font-semibold text-slate-700">
      {label}
    </span>
  )
}
