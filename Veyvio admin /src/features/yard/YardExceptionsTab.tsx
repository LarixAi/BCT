  ...
  return (
    <div className="grid gap-4 xl:grid-cols-[1fr_280px]">
      <SectionCard title="Operational exceptions" description={`${open.length} issues requiring intervention`}>
        {open.length === 0 ? (
          <p className="text-sm text-muted">No open exceptions at this depot.</p>
        ) : (
              ...
              <li key={ex.id} className={`rounded-lg border p-3 ${SEVERITY_STYLES[ex.severity]}`}>
                <div className="flex flex-wrap items-start justify-between gap-2">
                  <div>
                    ...
                    <p className="text-sm text-ink-soft">{ex.detail}</p>
                  </div>
                  <span className="rounded-full bg-surface/80 px-2 py-0.5 text-xs font-medium uppercase text-ink-soft">{ex.severity}</span>
                </div>
