...
export function TimeOffPage() {
  ...
  if (isLoading) return <p className="text-sm text-slate-500">Loading leave requests…</p>
              ...
              {selected.status !== 'cancelled' && selected.status !== 'rejected' && (
                <div className="space-y-2 rounded-lg border border-slate-200 p-3">
                  ...
                  <button
                    ...
                    className="rounded-lg border border-slate-300 px-3 py-1.5 text-sm font-medium text-slate-800 hover:bg-slate-50 disabled:opacity-60"
                  >
                    Cancel leave
                  </button>
                </div>
