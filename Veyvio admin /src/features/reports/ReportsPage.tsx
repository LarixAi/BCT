...
function availabilityLabel(status: ReportAvailability) {
  ...
  return { text: 'Planned', className: 'bg-surface-muted text-ink-soft border-border' }
}
  ...
  return (
    <div className="space-y-6">
      <header className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <p className="text-xs font-semibold uppercase tracking-wide text-muted">Reporting centre</p>
          <h1 className="mt-1 text-2xl font-semibold text-ink">Reports</h1>
          <p className="mt-1 text-sm text-ink-soft">
            Shared Command reporting from Driver, Yard, Maintenance and Admin actions · {depotLabel} ·{' '}
            {operationalDate}
          </p>
          <p className="mt-1 text-xs text-muted">{refreshLabel}</p>
        </div>
        <Link
          to="/reports/daily-operations"
          className="inline-flex items-center gap-2 rounded-full bg-midnight px-4 py-2.5 text-sm font-semibold text-white hover:bg-command-800"
        >
      ...
      <section>
        <h2 className="text-sm font-semibold text-ink">Key reports</h2>
        <div className="mt-3 grid gap-3 md:grid-cols-2">
          {keyReports.map((report) => {
            ...
            const body = (
              <div className="flex h-full flex-col rounded-xl border border-border bg-surface p-4 shadow-sm transition hover:border-border-strong">
                <div className="flex items-start justify-between gap-3">
                  <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-surface-muted">
                    <FileBarChart className="h-5 w-5 text-ink-soft" />
                  </div>
                  <span className={`rounded-full border px-2 py-0.5 text-[11px] font-semibold ${badge.className}`}>
                    {badge.text}
                  </span>
                </div>
                <p className="mt-3 font-semibold text-ink">{report.title}</p>
      ...
      <section className="grid gap-4 lg:grid-cols-2">
        <div className="rounded-xl border border-border bg-surface p-4">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-sm font-semibold text-ink">Requires attention</h2>
          </div>
      ...
      <section className="grid gap-4 lg:grid-cols-2">
        <div className="rounded-xl border border-border bg-surface p-4">
          ...
          <ul className="mt-3 space-y-2">
            {SAVED_REPORT_STUBS.map((item) => (
              <li key={item.id}>
                <Link to={item.href} className="block rounded-lg px-2 py-2 text-sm text-ink hover:bg-surface-muted">
                  {item.title}
                </Link>
              </li>
            ))}
        ...
        <div className="rounded-xl border border-border bg-surface p-4">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-sm font-semibold text-ink">Scheduled reports</h2>
          </div>
          <ul className="mt-3 space-y-2">
            {SCHEDULED_REPORT_STUBS.map((item) => (
              <li key={item.id} className="flex items-start justify-between gap-3 rounded-lg px-2 py-2">
                <div>
                  <Link to={item.href} className="text-sm font-medium text-ink hover:underline">
                    {item.title}
                  </Link>
      ...
      <section>
        <div className="flex flex-wrap items-center justify-between gap-3">
          <h2 className="text-sm font-semibold text-ink">Report catalogue</h2>
                  ...
                  <tr key={report.id} className="border-b border-border last:border-0">
                    ...
                    <td className="px-4 py-3 text-right">
                      {report.availability === 'planned' ? (
                        <span className="text-xs text-muted">Not built yet</span>
                      ) : (
                        <Link to={report.href} className="text-sm font-semibold text-ink hover:underline">
                          Open
                        </Link>
                      )}
                    </td>
...
function Kpi({
  ...
  return (
    <Link to={href} className="rounded-xl border border-border bg-surface p-4 shadow-sm hover:border-border-strong">
      <p className="text-xs font-medium uppercase tracking-wide text-muted">{label}</p>
      <p className="mt-1 text-3xl font-bold tabular-nums text-ink">{value}</p>
      <p className="mt-2 text-xs font-semibold text-muted">View records →</p>
    </Link>
  )
}
