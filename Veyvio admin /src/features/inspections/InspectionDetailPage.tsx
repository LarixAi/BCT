...
export function InspectionDetailPage() {
  ...
  if (isLoading) return <p className="text-sm text-muted">Loading inspection…</p>
    ...
    <div className="space-y-6">
      <div>
        ...
        <div className="mt-2 flex flex-wrap items-start justify-between gap-3">
          <div>
            <h1 className="text-2xl font-semibold tabular-nums text-ink">
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <div className="rounded-xl border border-border bg-surface p-3">
          ...
          <p className="text-xs text-muted">{daysRemainingLabel(inspection.dueDate)}</p>
        </div>
        <div className="rounded-xl border border-border bg-surface p-3">
        ...
        <div className="rounded-xl border border-border bg-surface p-3">
          <p className="text-xs text-muted">Provider / inspector</p>
          <p className="font-medium text-ink">{inspection.provider}</p>
          <p className="text-xs text-muted">{inspection.inspectorName ?? 'Not assigned'}</p>
        </div>
      </div>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <SectionCard title="Defects raised" description="Shared defect records from this inspection">
          {inspection.linkedDefects.length === 0 ? (
            <p className="text-sm text-muted">No defects linked.</p>
          ) : (
            <ul className="space-y-2 text-sm">
              {inspection.linkedDefects.map((d) => (
                <li key={d.defectId} className="rounded-lg border border-border px-3 py-2">
        ...
        <SectionCard title="Linked maintenance work orders" description="Rectification owned by Maintenance">
          {inspection.linkedWorkOrders.length === 0 ? (
            <p className="text-sm text-muted">No work orders linked.</p>
          ) : (
            <ul className="space-y-2 text-sm">
              {inspection.linkedWorkOrders.map((w) => (
                <li key={w.workOrderId} className="rounded-lg border border-border px-3 py-2">
                  <Link
                  ...
                  <p className="text-xs text-muted">{w.status}</p>
        ...
        {inspection.status === 'signed_off' ? (
          ...
          <>
            {blockers.length > 0 ? (
              ...
              <p className="mb-3 text-sm text-ink-soft">All sign-off checks are clear.</p>
            )}
            <button
              type="button"
