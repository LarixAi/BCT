  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <h1 className="mt-2 text-2xl font-semibold text-slate-900">VOR board</h1>
        <p className="text-sm text-slate-600">Vehicles off road or in workshop — replacement planning and release tracking</p>
      </div>
      ...
      <SectionCard title="Off road vehicles" description={`${vorVehicles.length} vehicles`}>
        {isLoading ? (
          ...
          <p className="text-sm text-slate-500">No vehicles currently VOR.</p>
        ) : (
              ...
              <li key={v.id} className="rounded-lg border border-red-100 bg-red-50/50 px-4 py-3">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div>
                    ...
                    <p className="text-sm text-slate-600">{v.reference} · {v.currentDepotName}</p>
                ...
                {v.vorRecords.filter((r) => !r.resolvedAt).map((r) => (
                  <p key={r.id} className="mt-2 text-xs text-slate-600">
