            ...
            {hub.templates.map((t) => (
              <tr key={t.id} className="border-b border-border/60">
                <td className="py-2.5 pr-3">
                  ...
                  <p className="text-xs text-muted">{t.vehicleCategories.join(', ')}</p>
                </td>
                <td className="py-2.5 pr-3">{t.version}</td>
                <td className="py-2.5 pr-3 text-ink-soft">{t.frequency}</td>
        ...
        <ul className="space-y-2 text-sm">
          {hub.templates.flatMap((t) =>
            t.escalationRules.map((r) => (
              <li key={`${t.id}-${r}`} className="flex justify-between rounded-lg border border-border px-3 py-2">
                <span>{t.name}</span>
                <span className="text-muted">{r.replace(/_/g, ' ')}</span>
              </li>
            )),
          )}
        </ul>
  ...
  return (
    <div className="grid gap-4 lg:grid-cols-2">
      <SectionCard title="Compliance intelligence" description={`${intel.suspiciousChecksToday} suspicious checks flagged today`}>
        <h3 className="mb-2 text-xs font-semibold uppercase text-muted">Recurring defect vehicles</h3>
        ...
        <h3 className="mb-2 text-xs font-semibold uppercase text-muted">Driver check quality</h3>
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {intel.depotComparison.map((d) => (
              <tr key={d.depotName} className="border-b border-border/60">
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {intel.templatePerformance.map((t) => (
              <tr key={t.templateName} className="border-b border-border/60">
