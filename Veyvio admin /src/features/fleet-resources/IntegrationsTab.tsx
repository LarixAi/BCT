    ...
    <div className="space-y-4">
      ...
      <div className="grid gap-3 md:grid-cols-2">
        {hub.integrations.map((i) => (
          <article key={i.id} className="rounded-xl border border-border bg-surface px-4 py-3">
            <div className="flex items-start justify-between gap-2">
              <div>
                <p className="text-xs font-medium uppercase tracking-wide text-muted">
                  {INTEGRATION_CATEGORY_LABEL[i.category]}
                </p>
                <h3 className="mt-0.5 text-base font-semibold text-ink">{i.provider}</h3>
              </div>
              <span
            ...
            <p className="mt-2 text-sm text-ink-soft">{i.notes}</p>
            ...
            <button
              type="button"
              className="mt-3 rounded-lg border border-border px-3 py-1.5 text-xs font-medium text-ink-soft hover:bg-surface-muted"
            >
