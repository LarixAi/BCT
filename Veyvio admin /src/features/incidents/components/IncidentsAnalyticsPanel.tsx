  ...
  return (
    <SectionCard title="Incident analytics" description="Trends, causes and prevention reporting">
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-3">
        <div>
          <h3 className="text-xs font-semibold uppercase text-muted">By type</h3>
        ...
        <div>
          <h3 className="text-xs font-semibold uppercase text-muted">By severity</h3>
          <ul className="mt-2 space-y-1 text-sm">
        ...
        <div>
          <h3 className="text-xs font-semibold uppercase text-muted">By depot</h3>
          <ul className="mt-2 space-y-1 text-sm">
...
function Stat({ label, value, tone = 'neutral' }: { label: string; value: string | number; tone?: 'neutral' | 'danger' }) {
  return (
    <div className="rounded-lg border border-border bg-surface-muted/50 p-3">
      <p className={`text-xl font-bold tabular-nums ${tone === 'danger' ? 'text-red-800' : 'text-ink'}`}>{value}</p>
