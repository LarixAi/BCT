...
export async function POST(
  ...
  try {
    ...
    const result = await fetchCommandMutation<Record<string, unknown>>(
      accessToken,
      `/executive/documents/${encodeURIComponent(id)}/download`,
      input,
      gateway.session.security.id,
      gateway.requestId,
    );
    // Short-lived signed URL is intentional; strip any permanent storage fields.
    const safe = { ...result };
    delete safe.storageKey;
    delete safe.permanentUrl;
