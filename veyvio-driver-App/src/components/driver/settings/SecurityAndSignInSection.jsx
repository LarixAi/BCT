...
/**
 * Security and sign-in — biometric unlock and trusted device management.
 * Rendered as a section inside Settings rather than its own page.
 */
export default function SecurityAndSignInSection({ driver }) {
  ...
  return (
    <section>
      <DriverSectionTitle>Security and sign-in</DriverSectionTitle>
      ...
      <div className={`flex items-start gap-3 p-4 ${op.card}`}>
        ...
        <div className="min-w-0 flex-1">
          <p className="text-[15px] font-semibold">Passkeys and biometrics</p>
          <p className="mt-1 text-sm text-muted-foreground leading-relaxed">
            Use Face ID, fingerprint or device security to unlock Veyvio Driver on this phone. Your
            biometric information stays on the device — Veyvio never receives it.
          </p>
        </div>
        <SettingsToggle
      ...
      {prefs.enabled ? (
        <p className="mt-3 text-sm text-muted-foreground">
          Enabled on this device · {label}. After you sign out, the sign-in screen shows Continue with{" "}
          {label}.
        </p>
      ) : native && availability.available ? (
        <p className="mt-3 text-sm text-muted-foreground">
          Set up on this device after a successful password sign-in. Turning the toggle on stores a
          protected session unlock — not your password.
        </p>
      ) : null}
