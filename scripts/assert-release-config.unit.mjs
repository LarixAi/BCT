#!/usr/bin/env node
/**
 * Unit checks for Yard assert-release-config.mjs
...
const good = mkdtempSync(join(tmpdir(), "veyvio-yard-assert-good-"));
...
const bad = mkdtempSync(join(tmpdir(), "veyvio-yard-assert-bad-"));
